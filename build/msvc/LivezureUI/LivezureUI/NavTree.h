...
class NavTreeUI : public CListUI
{
public:
    ...
    NavTreeUI() : _root(NULL), m_dwDelayDeltaY(0), m_dwDelayNum(0), m_dwDelayLeft(0)
    {
		...
		_root->data()._nItemID = -1;
		_root->data()._nLayoutID = -1;
    }

    ~NavTreeUI() { if(_root) delete _root; }
    ...
    Node* AddNode(int nItemID, int nLayoutID, LPCTSTR text, Node* parent = NULL)
    {
		...
		node->data()._nItemID = nItemID;
		node->data()._nLayoutID = nLayoutID;
        ...
        if (node->data()._level == 0) {
            if (node->data()._expand) html_text += _T("<a><i navtree/tree_expand.png 2 0></a>");
			else html_text += _T("<a><i navtree/tree_expand.png 2 1></a>");
			html_text += _T("{f 1}{c #000000}");
        ...
        if (node->data()._level == 0) {
            pListElement->SetBkImage(_T("navtree/tree_top.png"));
        }
    ...
    void ExpandNode(Node* node, bool expand)
    {
		...
		if (node->data()._level == 0) {
			...
			node->data()._pListElement->SetFixedHeight(26);
		}
		else
		{
			html_text += _T("<x 16>");
			...
			html_text += _T("{/f}{/c}");
			node->data()._pListElement->SetText(html_text);
			node->data()._pListElement->SetFixedHeight(24);
		}
        ...
        for (int i = begin->data()._pListElement->GetIndex(); i <= end->data()._pListElement->GetIndex(); ++i) {
            ...
            if (_tcscmp(control->GetClass(), _T("ListLabelElementUI")) == 0) {
                Node* local_parent = ((NavTreeUI::Node*)control->GetTag())->parent();
