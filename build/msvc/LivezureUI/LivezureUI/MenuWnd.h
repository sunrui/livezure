...
public:
    ...
    void RePaint() 
    {
        ...
        CRenderEngine::DrawImageString(hDcBackground, &m_pm, rcClient, rcClient, _T("file='menu/menu_bk.png' corner='40,8,8,8'"), NULL);
...
public:
	...
	void OnShow(POINT pt)
	{
		...
		CListContainerElementUI* pCreate = static_cast<CListContainerElementUI *>(m_pm.FindControl(_T("menu_create")));
		CListContainerElementUI* pDelete = static_cast<CListContainerElementUI *>(m_pm.FindControl(_T("menu_delete")));
		CListContainerElementUI* pModify = static_cast<CListContainerElementUI *>(m_pm.FindControl(_T("menu_modify")));
		CListContainerElementUI* pDetail = static_cast<CListContainerElementUI *>(m_pm.FindControl(_T("menu_detail")));

		pCreate->SetEnabled();
		pCreate->Select(FALSE);
		pDelete->SetEnabled();
		pDelete->Select(FALSE);
		pModify->SetEnabled();
		pModify->Select(FALSE);
		pDetail->SetEnabled();
		pDetail->Select(FALSE);
    ...
    void Notify(TNotifyUI& msg)
    {
        if (msg.sType == _T("itemselect")) {
			...
			if (msg.pSender->GetName() == _T("menu_create")) {
				if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_create"), 0, 0, true);
			}
			if (msg.pSender->GetName() == _T("menu_modify")) {
				if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_modify"), 0, 0, true);
			}
			if (msg.pSender->GetName() == _T("menu_delete")) {
				if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_delete"), 0, 0, true);
			}
			if (msg.pSender->GetName() == _T("menu_detail")) {
				if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_detail"), 0, 0, true);
			}
    ...
    LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        CControlUI* pRoot = builder.Create(_T("list_menu.xml"), (UINT)0, NULL, &m_pm);
