...
hashmap_create(unsigned int nbuckets)
{
	...
	ptr->size = nbuckets;
	ptr->buckets = calloc(nbuckets, sizeof(struct hashbucket_s));
	if (!ptr->buckets) {
		free(ptr);
		return NULL;
	}
 ...
 */
int
hashmap_insert(hashmap_t map, const char *key, const void *data, unsigned int len)
{
	struct hashentry_s *ptr;
	int hash;
	char *key_copy;
 ...
 *          count found
 */
unsigned int hashmap_search(hashmap_t map, const char *key)
{
	int hash;
	struct hashentry_s *ptr;
